...
const Wallet = () => { 
  return (
   ...
   <p>For any persistent errors unrelated to this issue, contact our support with details regarding your error and any additional information you're able to gather. </p> </UncontrolledCollapse>
    ...
    <UncontrolledCollapse toggler="wallet2" style={{marginLeft: 50, marginBottom: 0}}><p>blindmixer uses BIP39 for seed derivation, which makes use of mnemonic words plus an additional optional phrase.
   ...
   <p>Please be aware that this is a two-way street; adding an additional phrase to your wallet also means that you will need that phrase to recover your wallet, as just the mnemonic phrase will not be sufficient.</p> </UncontrolledCollapse>
    ...
    <UncontrolledCollapse toggler="wallet3" style={{marginLeft: 50, marginBottom: 0}}><p>blindmixer and/ or other custodians cannot help with either of the above-described issues.
...
const WorkingsWallet = () => { 
  return ( 
    <React.Fragment>
    <div className="faq-header">
    <h2 id="Principles">Principles of the blindmixer wallet.
  ...
  One of the most fundamental principles of blindmixer is that all coins <i>(read: inputs)</i> are blinded. As a result, blindmixer cannot determine which inputs originally belonged to you, and which didn't. 
  It is therefore possible to receive your own inputs back. </p> 
  <p> If this is something you are bothered about, note that there is invariably a possibility that the link is not broken. This is not the or a focus of blindmixer.</p> 
  
  <p>Certain measures that could decrease the chances of you receiving your own inputs back could be: switching from <i>lightning</i> to <i>normal</i> transactions and vice versa; waiting a certain amount of time for your original inputs to be used by other people, or spending radically different amounts. <code>These measures should not be confused with <i>literal</i> unlinkability!</code>. Again, this is not a priority for blindmixer, nor possible, so we cannot be of help with such problems. </p> 
  <p>Please read the next FAQ for a more detailed answer.</p> </UncontrolledCollapse>
  <div id="Workingswallet2" className="faq-linebreak"><h3> <FontAwesomeIcon icon={faQuestionCircle} />  Help! Does blindmixer offer on-chain unlinkability?!</h3>   </div>
  <UncontrolledCollapse toggler="Workingswallet2" style={{marginLeft: 50, marginBottom: 0}}> <p>This question is a bit of a rehash and or has a lot of common ground with the above-mentioned question, yet it is still important to make clear: <b>NO!</b> blindmixer does <b>NOT</b> offer on-chain unlinkability in the literal sense of the word.</p>
  <p> What blindmixer does offer, (that is under the most ideal conditions) is complete unlinkability within its own ecosystem. What exactly does this mean, you ask? For that we first need to explore what exactly unlinkability is, and how it's applied in this context. 
    </p>
...
of the attacker).</i> [2S]</p>
 <p> Within the blindmixer ecosystem (whereby the custodian is the attacker), we can speak to the variables that make this possible; that is, if our software is used correctly, this environment can be created. Partly as a result thereof, it is impossible for us to speak to the (literal) unlinkability outside of our ecosystem.
  ...
  <UncontrolledCollapse toggler="Workingswallet3" style={{marginLeft: 50, marginBottom: 0}}> <p>blindmixer has been designed in such a way that there should be, relatively speaking, immediate settlement of funds. 
  ...
  <UncontrolledCollapse toggler="Workingswallet4" style={{marginLeft: 50, marginBottom: 0}}>
    <p>This is possible. It could be that the custodian neglected to either credit or send out your transaction. blindmixer has been designed in such a way that this should be trivial to prove.</p>
...
<p>
   If the custodian provides you with your own authorization signature, but incorrect spending parameters, the signature should not verify, and cheating would be trivial to prove.
    ...
    <p>
  <strong>Note! Because of the fact that blindmixer is still in beta, there might be extreme edge-cases where custodians could cheat the client whereby it is impossible for the client to prove he was cheated upon.
...
<div id="biz1" className="faq-linebreak"><h3> <FontAwesomeIcon icon={faQuestionCircle} />  Help!? Uhm... blindmixer and or other custodians will  “take“ my money if I don't withdraw it before X date..?</h3> </div>
...
<UncontrolledCollapse toggler="biz2" style={{marginLeft: 50, marginBottom: 0}}><p>If your custodian has scheduled wipes, they should provide the wallet with a signed date if asked for it, which can be found both in your configuration, and <a href="https://mixer.blindmixer.com/faq">here.</a>
...
<UncontrolledCollapse toggler="biz3" style={{marginLeft: 50, marginBottom: 0}}><p>Please differentiate between the two most obvious scenario's: <i>Either you were too late</i>, in which case blindmixer will likely be unable to assist you. 
You could contact blindmixer (or the operator of the custodian) with a list of your unblinded coins along with the receipt(s) and magnitude(s), but there are no guarantees. (blindmixer will most likely publish a recovery policy in a little while which specifies how and in which timeframe you'll be able to recover your lost coins on their affiliated custodians.) </p> 
...
const Trust = () => { 
  ...
  <div id="trt1" className="faq-linebreak"><h3> <FontAwesomeIcon icon={faQuestionCircle} />  Help! I have to trust blindmixer and/ or the custodian operator blindly?</h3></div>
  ...
  <p>Do not use the blindmixer wallet as your primary or long storage wallet, but instead, recognize it as a wallet that can be used for day-to-day operations and/ or similar. 
  </p> </UncontrolledCollapse>
  {/* TODO */}
...
<div id="trt4" className="faq-linebreak"><h3> <FontAwesomeIcon icon={faQuestionCircle} /> Help! What data exactly can or is logged by the custodian?</h3> </div>
  ...
  <p>There are certain ways custodians could, up to an acceptable level of certainty, link your inputs and outputs together. This would for example be possible when you immediately after hooking in sweep your blinded coins out again. </p>
   ...
   <p>
    ...
    <p>
      blindmixer has recognized this issue, and we've implemented some slight improvements to combat this issue. In settings, you'll need to enable <code>Add a random delay when synchronizing, Add ghost coins when synchronizing</code>.
...
<UncontrolledCollapse toggler="desk2" style={{marginLeft: 50, marginBottom: 0}}><p>Luckily for you, we've kept this in mind. Head over to <a href='https://mixer.blindmixer.com'>our wallet</a>, right click, and save the HTML page. From now on, you can load the HTML file instead of <code>https://mixer.blindmixer.com</code>. 
<br/>
